...
function validId(id) {
    match.nodeType(id, 'Identifier', "identifier binding");
    var name = id.name;
...
Vp.module = function module(params, body) {
    ...
    sections.functions.forEach(function(funDecl) {
        var id = funDecl.id;
        if (!validId(id))
            fail("expected valid function name, got " + id.name);
        this._env.bind(id.name, functionType(funDecl));
    }, this);
...
Vp.local = function local(dtor) {
    var y = dtor.id.name;
    if (!validId(dtor.id))
        fail("expected valid local identifier, got " + y);
    if (!dtor.init)
