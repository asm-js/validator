...
Vp.validate = function validate(src) {
    ...
    if (params.length > 2 || !params.every(validId, this)) {
...
// (Expression) -> Type
Vp.extractType = function extractType(node) {
    switch (node.type) {
...
Vp.functionType = function functionType(funDecl) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        paramTypes[i] = this.extractType(stmt.expression.right);
    }
...
Vp.import = function import_(params, x, init, loc) {
    ...
    switch (init.type) {
      case 'MemberExpression':
        ...
        match.identifier(init.object, e, "import environment", this);
      ...
      case 'NewExpression':
        ...
        match.identifier(callee.object, e, "import environment", this);
