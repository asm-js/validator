...
Vp.validate = function validate(src) {
    var module = esprima.parse("(" + src + ")").body[0].expression;
    ...
    return this.module(module.params.map(function(id) { return id.name; }), module.body.body);
};
    ...
    switch (node.type) {
      case 'BinaryExpression':
        switch (node.operator) {
          case '|':
            match.literal(node.right, 0, "int32 coercion");
            return types.signed;

          case '>>>':
            match.literal(node.right, 0, "uint32 coercion");
      ...
      case 'Literal':
        ...
        return node.raw.indexOf(".") === -1 ? types.int : types.double;
...
function extractReturnType(node) {
    return node ? extractType(node) : types.void;
}
...
function functionType(funDecl) {
    ...
    var returns = find(funDecl.body, function(node) { return node.type === 'ReturnStatement'; })
